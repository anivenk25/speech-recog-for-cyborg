...
def recognize_speech_from_file(audio_file_path, language='en-IN'):
    ...
    try:
        if language == 'en-IN':
            ...
            st.write("Listening for Odia speech...")
        else:
            recognized_text = None
            st.write("Language not supported")
...
def translate_text(text, dest_language='en'):
    ...
    Returns:
        str: Translated text.
    """
    translated_text = GoogleTranslator(source='hi', target=dest_language).translate(text)
...
if uploaded_file:
    ...
    audio_text = recognize_speech_from_file(uploaded_file)
    if audio_text:
        st.write("Recognized Text:", audio_text)

        translated_text = translate_text(audio_text, dest_language='en')
        if translated_text:
            ...
            st.write("Meeting minutes saved to file.")
